...
export class PrRunsService {
  ...
  private formatRun(run: Awaited<ReturnType<typeof this.findOneRaw>>) {
    ...
    return {
      ...
      generatedTestFiles: run.generatedTests
        ...
        passed: t.passed,
        category: t.testCategory ?? 'UNIT_TEST_WORTHWHILE',
        integrationTestReason:
          t.testCategory === 'INTEGRATION_TEST_NEEDED' ?
            t.integrationTestReason
          : null,
        downloadUrl: `/pr-runs/${run.id}/tests/${t.id}`,
        fileContent: this.resolveArtifactFileContent(t.content),
      })),
  ...
  /** Test source when generated; otherwise classification reason or existing repo tests. */
  private resolveArtifactFileContent(content: string | null | undefined): string | null {
