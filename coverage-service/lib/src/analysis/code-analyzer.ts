...
export type JsTestRunner = 'jest' | 'node:test';

/** True when the repo is configured to run tests with Jest. */
export function repoUsesJest(repoDir: string): boolean {
  ...
  if (existsSync(pkgPath)) {
    try {
      ...
      if (pkg.dependencies?.jest || pkg.devDependencies?.jest) return true;
      const testScript = pkg.scripts?.test ?? '';
      if (/\bjest\b/.test(testScript)) return true;
    } catch {
  ...
  for (const configName of [
    ...
    if (existsSync(join(repoDir, configName))) return true;
  }

  return false;
}

export function detectJsTestRunner(repoDir: string): JsTestRunner {
  return repoUsesJest(repoDir) ? 'jest' : 'node:test';
}
...
export function detectFramework(
  ...
  if (testCommand.includes('jest') || repoUsesJest(repoDir)) return 'jest';
