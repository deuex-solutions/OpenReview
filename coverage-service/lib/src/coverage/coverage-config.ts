...
/** When effective diff coverage is at or below this %, use full-file context for test generation. Default 50. */
export function getFullFileGenerationTriggerPercent(): number {
  const raw = process.env.FULL_FILE_GENERATION_THRESHOLD?.trim();
  if (!raw) return 50;
