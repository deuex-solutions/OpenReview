import type { ChangedFile, DiffCoverageReport, FileCoverage } from '../types';
import { pathsMatch } from './cobertura-parser';
import { meetsThreshold } from './coverage-workflow';
...
export function effectiveFileCoveragePercent(
  entry: FileCoverage | undefined,
): number {
  if (!entry) return 0;
  return entry.diffCoveragePercent ?? entry.lineCoveragePercent;
}

/** Uncovered line numbers to target when generating tests. */
export function resolveUncoveredLinesForGeneration(
  filePath: string,
  report: DiffCoverageReport,
  useEntireFile: boolean,
): number[] {
  const entry = report.fileCoverage.find((f) => pathsMatch(f.file, filePath));
  if (!entry) {
    return report.uncoveredLines
      .filter((l) => pathsMatch(l.file, filePath))
      .map((l) => l.line);
  }

  if (useEntireFile) {
    const allLines = entry.allUncoveredLines?.length
      ? entry.allUncoveredLines
      : entry.uncoveredLines;
    return [...new Set(allLines)].sort((a, b) => a - b);
  }

  if (entry.uncoveredLines.length > 0) {
    return entry.uncoveredLines;
  }
...
/** Skip further generation when a passing generated test already brought this file above threshold. */
export function isFileSatisfiedForGeneration(
  filePath: string,
  report: DiffCoverageReport,
  thresholdPercent: number,
  generatedTestPassed: boolean,
): boolean {
  if (!generatedTestPassed) return false;

  const entry = report.fileCoverage.find((f) => pathsMatch(f.file, filePath));
  if (!entry) return false;

  return meetsThreshold(effectiveFileCoveragePercent(entry), thresholdPercent);
}
