...
import { parseCoberturaXml, parseDiffCoverTotals, pathsMatch } from './cobertura-parser';
...
/** Infer how many changed lines diff-cover counted for a file (null when unknown). */
function inferDiffFileLineCount(entry: FileCoverage): number | null {
  const pct = entry.diffCoveragePercent;
  if (pct === null) return null;

  const missing = entry.uncoveredLines.length;
  if (missing > 0 && pct < 100) {
    return Math.max(missing, Math.round(missing / (1 - pct / 100)));
  }
  if (missing === 0 && pct >= 100) {
    return null;
  }
...
function recomputeDiffCoverageAfterExclusion(
  included: FileCoverage[],
  excluded: FileCoverage[],
  report: DiffCoverageReport,
): number {
  const excludedWithDiff = excluded.filter(
    (f) => f.diffCoveragePercent !== null,
  );

  if (excludedWithDiff.length === 0) {
    return report.diffCoveragePercent;
  }

  const rawTotals = parseDiffCoverTotals(report.rawOutput ?? '');
  if (!rawTotals) {
    return report.diffCoveragePercent;
  }

  let excludedKnownTotal = 0;
  for (const entry of excludedWithDiff) {
    const lineCount = inferDiffFileLineCount(entry);
    if (lineCount !== null) {
      excludedKnownTotal += lineCount;
    }
  }

  const includedMissing = included
    .filter((f) => f.diffCoveragePercent !== null)
    .reduce((sum, f) => sum + f.uncoveredLines.length, 0);

  const adjustedTotal = rawTotals.total - excludedKnownTotal;
  if (adjustedTotal <= 0) {
    return report.diffCoveragePercent ?? 100;
  }

  return ((adjustedTotal - includedMissing) / adjustedTotal) * 100;
}

/** Remove integration-test-only files from coverage metrics and recompute totals. */
export function excludeFilesFromCoverageReport(
  report: DiffCoverageReport,
  ...
  const isExcluded = (file: string) =>
    excludedPaths.some((p) => pathsMatch(p, file));

  const excluded = report.fileCoverage.filter((f) => isExcluded(f.file));
  const fileCoverage = report.fileCoverage.filter((f) => !isExcluded(f.file));
  const uncoveredLines = report.uncoveredLines.filter(
    (l) => !isExcluded(l.file),
  );
  const filesWithPoorCoverage = report.filesWithPoorCoverage.filter(
    (f) => !isExcluded(f),
  );

  const diffCoveragePercent = recomputeDiffCoverageAfterExclusion(
    fileCoverage,
    excluded,
    report,
  );
  ...
  const totalCoveragePercent =
    fileCoverage.length > 0
      ? fileCoverage.reduce((sum, f) => sum + f.lineCoveragePercent, 0) /
        fileCoverage.length
      : 0;
