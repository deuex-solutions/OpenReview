...
export class OpenAIProvider implements TestGenerationProvider {
  ...
  async classifyTestability(
        ...
        messages: [
          {
            role: 'system',
            content:
              'You classify source files for test strategy. Respond with JSON only.',
          },
    ...
    const parsed = parseTestabilityClassificationResponse(raw);
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: buildTestGenerationSystemPrompt(context.language, context.framework),
          },
          { role: 'user', content: prompt },
        ],
        temperature: 0.1,
