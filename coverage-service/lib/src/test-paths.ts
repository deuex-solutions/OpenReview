...
export function nestedJavaScriptTestPaths(sourceFile: string): string[] {
  ...
  const paths: string[] = [];
  for (const folder of ['__tests__', '__test__'] as const) {
...
/** True when a test file path targets the same basename as the source file. */
export function testFileBasenameMatchesSource(
  testPath: string,
  sourceFile: string,
): boolean {
  const sourceBase = sourceFile.split('/').pop()!.replace(/\.[^.]+$/, '');
  const fileName = normalizeRepoPath(testPath).split('/').pop() ?? testPath;
  const testBase = fileName.replace(/\.(test|spec)\.(tsx|ts|jsx|js)$/, '');
  if (testBase.startsWith('test_')) {
    return testBase.slice(5) === sourceBase;
  }
  return testBase === sourceBase;
}

/** Infer a test file path for JS/TS — prefers src/.../__tests__/ (Jest roots). */
export function inferJavaScriptTestFilePath(sourceFile: string): string {
  const nested = nestedJavaScriptTestPaths(sourceFile);
  const colocated = nested.find((p) => p.includes('/__tests__/'));
...
/** All plausible test paths for a source file (__tests__ colocated first). */
...
export function pickExistingTestFilePath(
  ...
  const underTests = existingRelativePaths.filter((p) => {
    ...
    return (
      (n.startsWith('tests/') || n.startsWith('test/')) &&
      testFileBasenameMatchesSource(n, sourceFile)
    );
  });
...
/** True when stored content looks like generated test source (not empty or integration prose). */
export function hasRunnableTestContent(content?: string | null): boolean {
  const trimmed = content?.trim();
  if (!trimmed) return false;
  return /(?:^|\n)\s*(?:import |from |describe\(|it\(|test\(|def test_|@pytest|node:test)/m.test(
    trimmed,
  );
}
