...
export interface FileCoverage {
  ...
  /** Uncovered lines in the PR diff (from diff-cover). */
  uncoveredLines: number[];
  /** All uncovered lines in the file from Cobertura line coverage. */
  allUncoveredLines?: number[];
}
...
export interface GeneratedTest {
  ...
  content: string;
  targetFile: string;
  /** Whether the LLM classified this file as unit-testable or integration-test-only. */
  testCategory?: GeneratedTestCategory;
  /** Human-readable reason when testCategory is INTEGRATION_TEST_NEEDED. */
  integrationTestReason?: string;
}
...
export interface TestGenerationContext {
  ...
  /** When true, target all uncovered lines in the file (not only diff lines) and analyze the entire file. */
