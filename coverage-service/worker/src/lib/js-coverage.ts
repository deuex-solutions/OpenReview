...
import { sourceFileExtension, type ChangedFile, type RepositoryProvider, detectJsTestRunner, type JsTestRunner } from '@openreview/coverage-lib';
...
function buildJestRunTarget(
  ...
  if (withCoverage) {
    ...
    for (const p of sourcePaths) {
      parts.push(`--collectCoverageFrom=${shellQuote(escapeGlobPathForC8(p))}`);
    }
  }

  if (testPaths.length > 0) {
    parts.push(...testPaths.map((p) => shellQuote(p)));
  } else if (sourcePaths.length > 0) {
    parts.push('--findRelatedTests');
    parts.push(
      ...sourcePaths.map((p) => {
        const normalized = normalizePath(p);
        return shellQuote(normalized.startsWith('./') ? normalized : `./${normalized}`);
      }),
    );
  } else {
...
function buildRunTarget(
  ...
  if (runner === 'jest') {
    return buildJestRunTarget(sourcePaths, testPaths, false);
  }
...
export function buildJsCoverageCommand(
  ...
  const dir = repoDir ?? '.';
  const runner = detectJsTestRunner(dir);

  if (runner === 'jest') {
    return buildJestRunTarget(sourcePaths, testPaths, true);
  }

  const runTarget = wrapForCoverageInstrumentedChild(
    buildRunTarget(dir, sourcePaths, testPaths, runner),
  );
...
export function buildJsTestCommand(
  ...
  const dir = repoDir ?? '.';
  const runner = detectJsTestRunner(dir);
  return buildRunTarget(dir, sourcePaths, testPaths, runner);
}

/** True when the test runner reported success but executed zero tests. */
export function isJsTestOutputEmpty(
  runner: JsTestRunner,
  output: string,
  exitCode: number,
): boolean {
  ...
  if (runner === 'jest') {
    return (
      /Tests:\s+0 total/.test(output) ||
      /No tests found/i.test(output) ||
      /Test Suites:\s+0 total/.test(output)
    );
  }

  return /# tests 0\b/m.test(output);
}

export function getJsTestRunner(repoDir: string): JsTestRunner {
  return detectJsTestRunner(repoDir);
}
...
function isTestCompatible(
  repoDir: string,
  testPath: string,
  runner: JsTestRunner,
): boolean {
  if (runner === 'jest') return true;
  return isNodeTestCompatible(repoDir, testPath);
}
...
export async function collectJsTestPaths(
  ...
  const runner = detectJsTestRunner(repoDir);
  const framework = runner === 'jest' ? 'jest' : 'node:test';
  ...
  for (const file of changedFiles) {
    ...
    if (isTestFile(file.path)) {
      ...
      if (isTestCompatible(repoDir, normalized, runner)) {
  ...
  for (const sourcePath of sourcePaths) {
    ...
    for (const testPath of existing) {
      ...
      if (isTestCompatible(repoDir, rel, runner)) {
        testPaths.add(rel);
      }
    }

    for (const candidate of guessTestFileNames(sourcePath)) {
      if (existsSync(join(repoDir, candidate)) && isTestCompatible(repoDir, candidate, runner)) {
  ...
  if (testPaths.size === 0 && sourcePaths.length > 0) {
    ...
    for (const testPath of allTests) {
      if (
        isTestCompatible(repoDir, testPath, runner) &&
