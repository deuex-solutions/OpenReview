...
function usesJestRunner(content: string): boolean {
  return (
    /\bjest\.(fn|mock|spyOn|useFakeTimers)\b/.test(content) ||
    (/\bdescribe\s*\(/.test(content) &&
      !/\bfrom\s+['"]node:test['"]/.test(content))
  );
}

function repoProvidesJsdom(repoPackages: string[]): boolean {
  return repoPackages.some((pkg) => {
    const n = normalizePackageName(pkg);
    return n === 'jsdom' || n === 'jest-environment-jsdom';
  });
}
...
export function prepareGeneratedJsTestContent(content: string): string {
  if (
    !needsJsdomEnvironment(content) ||
    hasJsdomSetup(content) ||
    usesJestRunner(content)
  ) {
    return content;
  }
...
function inferJsTestPackages(
  ...
  if (
    needsJsdomEnvironment(combined) &&
    !repoProvidesJsdom(repoPackages) &&
    !usesJestRunner(combined)
  ) {
    packages.add('jsdom');
  }
...
function inferAutoTestPackages(
  testContents: string[],
  ecosystem: 'python' | 'javascript',
  repoPackages: string[] = [],
): string[] {
  return ecosystem === 'javascript'
    ? inferJsTestPackages(testContents, repoPackages)
...
export function collectTestRunDependencies(
  ...
  const autoDeps =
    ecosystem === 'javascript'
      ? [
          ...inferAutoTestPackages(testContents, ecosystem, repoPackages),
