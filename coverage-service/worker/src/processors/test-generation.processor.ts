...
export class TestGenerationProcessor {
  ...
  async process(
    ...
    try {
      ...
      const parsed =
        generated.testCategory === 'INTEGRATION_TEST_NEEDED' ?
          { content: generated.content, declaredDeps: [] as string[] }
        : parseGeneratedTestContent(generated.content);
      ...
      await prisma.generatedTestArtifact.create({
        data: {
          ...
          integrationTestReason: generated.integrationTestReason ?? null,
        },
      });
      ...
      if (generated.testCategory === 'INTEGRATION_TEST_NEEDED') {
        await this.log(
          data.runId,
          'info',
          `Integration test recommended for ${targetFile}: ${generated.integrationTestReason ?? generated.content}`,
        );
      } else {
        await this.log(
          data.runId,
  ...
  private async runCoverage(params: {
    ...
    if (useAutoJsCoverage && getJsTestRunner(params.runDir) === 'node:test') {
  ...
  private async generateTestForFile(params: {
    ...
    const effectiveCoverage = params.coverageReport
      ? getEffectiveCoverage(
          params.coverageReport.diffCoveragePercent,
          params.coverageReport.totalCoveragePercent,
        )
      : 0;
    const useEntireFile =
      effectiveCoverage <= getFullFileGenerationTriggerPercent();
    const uncoveredForFile = params.coverageReport
      ? resolveUncoveredLinesForGeneration(
          params.filePath,
          params.coverageReport,
          useEntireFile,
        )
      : [];
    ...
    const symbols = await analyzer.extractSymbols(
      source,
      params.filePath,
      useEntireFile ? [] : uncoveredForFile,
    );
    ...
    if (classification.usage) {
      await this.persistUsage(params.runId, null, classification.usage);
    }
    ...
    if (classification.category === 'INTEGRATION_TEST_NEEDED') {
      return {
        filePath:
          ...
          inferTestFilePath(params.filePath, framework),
        content: classification.reason,
        targetFile: params.filePath,
        testCategory: 'INTEGRATION_TEST_NEEDED' as const,
